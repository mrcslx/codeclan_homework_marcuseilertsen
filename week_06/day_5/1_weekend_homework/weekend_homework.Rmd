...
H0 : sleep total in sampled population is the same as for jabberwockies, approx. 7 hours per night
...
Our p-value is 0.996. The sleep total in the sampled population does not differ from the typical value for jabberwockies.
...
null_distribution_2 %>% 
  visualise() +
  shade_p_value(obs_stat = obs_stat_2, direction = "right")
```

```{r}
null_distribution_2 %>% 
  get_p_value(obs_stat = obs_stat_2, direction = "right")
```
...
null_distribution_3 <- msleep %>% 
  ...
  mutate(is_domesticated = coalesce(is_domesticated, FALSE)) %>% 
  mutate(is_domesticated = as.character(is_domesticated)) %>% 
...
null_distribution_3 %>%
  visualise(bins = 30) +
  shade_p_value(obs_stat = obs_stat_3, direction = "both")
```

```{r}
null_distribution_3 %>%
  get_p_value(obs_stat = obs_stat_3, direction = "both")
```
